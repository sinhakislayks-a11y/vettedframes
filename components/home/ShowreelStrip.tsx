      ...
      <div className="relative">
        ...
        >
          {items.map((item, i) => (
            <div
              key={`${item.title}-${i}`}
              className={`group relative flex-shrink-0 w-72 h-44 rounded-[4px] bg-gradient-to-br ${item.gradient} border border-border-custom overflow-hidden cursor-pointer`}
            >
              ...
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <div className="w-10 h-10 rounded-[4px] bg-brand/20 border border-brand/30 flex items-center justify-center">
